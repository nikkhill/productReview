...
features = [('Cpu/Processor/qualcomm/exynos/mediatek/snapdragon', ''),
            ...
            ('RAM/Memory', 'expandable/sdcard/card,available/internal/apps/ram'),
...
def score(item):
    s = abs(item[1])-(len(tool.check(item[0]))/float(len(nltk.word_tokenize(item[0]))))/3.0
    if len(item[0])>=100:
        s-=2
    return s
...
def printByFeatures(pros, cons, d):
    totscore = 0
    norm = 0
    for f in features:
        print "---------------"
